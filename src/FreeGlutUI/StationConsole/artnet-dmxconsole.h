// artnet-dmxconsole.h : Include file for standard system include files,
// or project specific include files.
#ifndef __ARTNETDMXCONSOLE_H_
#define __ARTNETDMXCONSOLE_H_
...
public:
    ...
    int display_mode = DISP_MODE_DMX;
    int current_channel = 0;	/* channel cursor is positioned on */
    int first_channel = 0;	/* channel in upper left corner */
    int channels_per_line = 80 / 4;
    int channels_per_screen = 80 / 4 * 24 / 2;
    int undo_possible = 0;
    int current_cue = 0;	/* select with F keys */
    float fadetime = 1.0f;
    int fading = 0;		/* percentage counter of fade process */
    int palette_number = 0;
    int palette[MAXCOLOR];
    char* errorstr = NULL;
    int channels_offset = 1;
...
# endif
    ...
    {
        ArtNetDmxConsole::Inst()->save("crash.chn");
        exit(1);
    }
    ...
    static void cleanup()
    {
        if (ArtNetDmxConsole::Inst()->w)
        {
            resetty();
            endwin();
        }

        artnet_stop(ArtNetDmxConsole::Inst()->node);

        if (ArtNetDmxConsole::Inst()->errorstr)
            puts(ArtNetDmxConsole::Inst()->errorstr);
    }
    ...
    int Run(int argc, char* argv[])
    {
       ...
       /*static struct option long_options[] = {
        ...
        // parse options
        while ((optc = getopt_long(argc, argv, "va:b:p:s:", NULL, NULL)) != EOF) {
            switch (optc) {
            case 'a':
...
public:
    static ArtNetDmxConsole* Inst() {
        static ArtNetDmxConsole ArtNetDmxConsoleInstance;
        return &ArtNetDmxConsoleInstance;
    }
};

#endif // !__ARTNETDMXCONSOLE_H_