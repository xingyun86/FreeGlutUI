// artnet-setdmx.h : Include file for standard system include files,
// or project specific include files.
#ifndef __ARTNETSETDMX_H_
#define __ARTNETSETDMX_H_
...
class ArtNetSetDmx {
public:
    const char* SHORT_NAME = "ArtNet Node";
    const char* LONG_NAME = "libartnet setdmx example";
...
#pragma pack()
    ...
    int do_fade(artnet_node node, opts_t* ops) {
        ...
        double fadetime = ops->fade_time;
        int val = ops->value;
        double p;
        uint8_t* dmx = (uint8_t*)malloc(chan * sizeof(uint8_t));
        if (dmx == NULL)
        {
            return 0;
        }
        ...
        while (t <= tend) {
            ...
            if (fadetime)
                p = ((double)(t - tstart) / 1000.0 / fadetime);
            else
                p = 1.0;

            dmx[chan] = (uint8_t)((int)(float)val * p);
...
public:
    static ArtNetSetDmx* Inst() {
        static ArtNetSetDmx ArtNetSetDmxInstance;
        return &ArtNetSetDmxInstance;
    }
};

#endif // !__ARTNETSETDMX_H_